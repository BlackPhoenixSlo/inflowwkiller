...
Three rules, each of which exists because of an observed production defect:
  ...
  1. **A rung stays EMPTY rather than take wrong content.** The caption is a
     ...
     prod), so an empty rung is safe; a mis-bound one is not. NovaFree has no
     partner content at all, so her collab rung is seeded empty on purpose.
  2. **No media id appears in two rungs.** On prod NovaFree has 26 ids bound to
  ...
  3. **Scarcest rung first.** Payoff content is rare (19 of NovaFree's 103 items
...
# the closed fields right — measured on NovaFree: 3 items are tagged
...
# NovaFree the photo batches (06-30 → 07-01) barely overlap the video batches
...
# $0. Measured on NovaFree: "Custom / exclusive" ($200 — "one of one, nobody
...
def item_band(row: dict[str, Any]) -> tuple[int, int | None]:
    ...
    if _has_flags(f) and explicit:
        ...
        # still covered below. On NovaFree the two flags disagree on 31 of 81
...
# every clothing- or act-gated rung matches nothing — on Ava's 1,361-item vault
