...
def _fold_clip_flags(per_frame: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    marked the whole clip uncovered. 20 of 21 NovaFree clips folded to
...
async def flags_all(payload: dict = Body(...)) -> dict[str, Any]:
    ...
    fell into it by default — on NovaFree that meant a penetration clip and a
...
async def flags_review_queue(account_id: str = "", limit: int = 120,
    ...
    and is not good enough to hide things behind: on NovaFree it flags 40 items
...
# cheaper over NovaFree, about $0.001 for a 103-item vault.
#
...
#
# GRADED against operator labels on 27 NovaFree items (`_probe_flags_score.py`).
