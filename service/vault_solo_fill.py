...
Three rules carried over from what production got wrong:
  ...
  2. **Never re-bind media another item already sells** — including script
     items. On prod one NovaFree media id sits in three rungs at three prices.
  3. **Believe the prose over the taxonomy.** Measured on NovaFree: 3 items are
...
# The fewest media that still make the word honest. "Set" is plural: NovaFree's
...
# Measured on NovaFree: "black lace deep fingering" ($100) went out over a clip
...
async def suggest_singles(account_id: str, *, only_empty: bool = True,
    ...
    # cost once the catalog outgrows the vault: on NovaFree, 18 rows over 101
    ...
    for t in targets:
        ...
        # NovaFree's 102 items clear `preview_ok`, and after 21 rows have taken
