...
import { Radio, Play, Eye, ChevronDown, Plus, Save, X, Image as ImageIcon } from "lucide-react";
...
function ruleToForm(rule: AutomationRule | null): Form {
  ...
  return {
    ...
    // relay reads it through `_common.bool_knob`. One reader, both sides.
...
export default function OnlineBlastTab() {
   ...
   *  Broadcasting to all online fans now…" from "Send now", and `err` carries
   *  "Run failed: …" and "Preview failed: …". A bar labelled Save that turns red
  ...
  return (
    <div className="space-y-5 max-w-2xl">
       ...
       *  tab — "✓ Broadcasting to all online fans now…" and "Run failed: …"
       *  from Send now, "Preview failed: …" from the preview — so a bar
       *  labelled Save that shows them narrates someone else's action, and a
       *  red one replaces the ✓ of the save that actually landed. The in-flow
       *  row still shows both where they belong. */}
      <StickySaveBar
        hostPadding={0}
        onSave={save}
        saving={busy}
