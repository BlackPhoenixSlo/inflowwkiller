 ...
 * ids from the live account (700937824869654528 / its fan 500465665494917120).
 */
import { describe, expect, it } from "vitest";

import { compareFanIds, fanIdFromParam, isValidFanId, lastMessageFromFan, sameUserId, type FanId } from "@/lib/fanId";

const FANSLY_ME = "700937824869654528";
const FANSLY_FAN = "500465665494917120";
const OF_ME = 1234567;
 ...
 * `Number(param)` in /chat/[accountId]/[fanId] rounded 900404711209099264 to
 * 900404711209099300 and the page requested a fan that does not exist ("↗ pop
...
describe("fanIdFromParam", () => {
  ...
  it("round-trips every id the popout is handed", () => {
    for (const raw of ["900404711209099264", "700937824869654528", "500465665494917120", "582113372", "1"]) {
...
describe("compareFanIds", () => {
  ...
  it("distinguishes snowflakes that float64 would collapse together", () => {
    ...
    const a = "900404711209099264";
    const b = "900404711209099265";
...
describe("isValidFanId", () => {
  it("accepts the shapes both platforms actually send", () => {
    ...
    expect(isValidFanId("900404711209099264")).toBe(true);
  });
